...
class YanExecutor:
    ...
    def __init__(self, yan_path=None):
        if yan_path is None:
            yan_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'main.py')
        self.yan_path = yan_path

    def execute(self, code: str) -> tuple:
        """
        执行言语言代码
        返回: (success: bool, output: list, error: str or None)
        """
        try:
            result = subprocess.run(
                [sys.executable, self.yan_path],
                input=code,
                capture_output=True,
                text=True,
                timeout=30,
